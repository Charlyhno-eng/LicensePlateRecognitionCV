...
# OCR avec orientation désactivée pour plus de rapidité
...
def display_camera():
    ...
    while True:
        ...
        if frame_count % recognition_frequency == 0:
            ...
            current_plate = recognize_license_plate(small_frame)

            if current_plate == last_plate and current_plate is not None:
                stable_count += 1
            else:
                stable_count = 1
                last_plate = current_plate

            if stable_count == stable_threshold and current_plate is not None:
                print("License Plate:", current_plate)
